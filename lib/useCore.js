...
function coreFromApi(playlists = []) {
  ...
  for (const p of playlists) {
    ...
    const entry = {
      ...
      isCurrentlyWatching: p.isCurrentlyWatching ?? false,
...
export function useCore() {
  ...
  const dispatch = useCallback(
    async (action) => {
      ...
      try {
        ...
        if (action.type === "currentlyWatching") {
          const currentlyWatching = action.currentlyWatching;
          const next = {
            ...coreRef.current,
            playlists: {
              ...coreRef.current.playlists,
              [id]: {
                ...coreRef.current.playlists[id],
                isCurrentlyWatching: currentlyWatching,
              },
            },
          };
          coreRef.current = next;
          ...
          await api(
            ...
            {
              method: "PATCH",
              body: JSON.stringify({ currentlyWatching }),
            },
            token
          );
          return null;
  ...
  return { core, dispatch, playlists, ready, busy: pending > 0 };
}