...
const BUNDLED_PATH = path.join(BIN_DIR, platformAssetName());
...
async function resolveYtDlpPath() {
  ...
  if (existsSync(BUNDLED_PATH) && isRealBinary(BUNDLED_PATH)) {
    return (cachedPath = BUNDLED_PATH);
  }
  try {
    const { constants } = require("youtube-dl-exec");
    if (existsSync(constants.YOUTUBE_DL_PATH) && isRealBinary(constants.YOUTUBE_DL_PATH)) {
